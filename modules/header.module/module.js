...
liElements.forEach(function(currentElement, index){ 
  ...
  ulElement.classList.add('submenu-closed');

  currentElement.children[0].classList.add('main-menu');


  currentElement.addEventListener("click", function(event){
    var classes = event.target.getAttribute("class");

    if((classes != null || classes != 'null') && (classes.includes("hs-menu-depth-1") || classes.includes("main-menu"))){
      if(ulElement.classList.contains('submenu-active') == false){
        removeActiveClass()
        ulElement.classList.remove('submenu-closed');
        ulElement.classList.add('submenu-active');
      } else {
        ulElement.classList.remove('submenu-active');
        ulElement.classList.add('submenu-closed');
      }
    } else {
      ulElement.classList.remove('submenu-active');
  ...
  function doSomething() {
    ...
    //     console.log(ulElement.classList.contains('submenu'))
  }

  function removeActiveClass(){
    var activeElement = document.querySelectorAll(".submenu-active")
